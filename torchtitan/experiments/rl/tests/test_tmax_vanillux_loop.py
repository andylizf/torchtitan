...
# --------------------------------------------------------------------------- #
# grade_tmax reward freshness: a pre-written reward must never be paid out.
# --------------------------------------------------------------------------- #


class _GradingSandbox:
    """Local-filesystem Sandbox for grading: exec runs real bash."""
...
def _grading_paths(monkeypatch: pytest.MonkeyPatch, tmp_path) -> dict:
    from torchtitan.experiments.rl.examples.tmax import grading

    tests_dir = tmp_path / "tests"
    verifier_dir = tmp_path / "logs" / "verifier"
    monkeypatch.setattr(grading, "_TESTS_DIR", str(tests_dir))
    monkeypatch.setattr(grading, "_TEST_SH", str(tests_dir / "test.sh"))
    monkeypatch.setattr(grading, "_VERIFIER_DIR", str(verifier_dir))
    return {
...
def test_grade_tmax_zeroes_pre_written_reward(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    from torchtitan.experiments.rl.examples.tmax import grading

    paths = _grading_paths(monkeypatch, tmp_path)
    paths["verifier_dir"].mkdir(parents=True)
    paths["reward"].write_text("1")  # the agent pre-paid itself

    tmax = {"test_sh": "true", "reward_path": str(paths["reward"])}
    reward = asyncio.run(
        grading.grade_tmax(_GradingSandbox(), tmax, workdir=paths["workdir"])
    )

    assert reward == 0.0


def test_grade_tmax_accepts_fresh_verifier_write(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    from torchtitan.experiments.rl.examples.tmax import grading
    ...
    paths = _grading_paths(monkeypatch, tmp_path)
    paths["verifier_dir"].mkdir(parents=True)
    paths["reward"].write_text("1")  # stale pre-write must not matter either way

    tmax = {
        "test_sh": f"printf 1 > {shlex.quote(str(paths['reward']))}",
        "reward_path": str(paths["reward"]),
    }
    reward = asyncio.run(
        grading.grade_tmax(_GradingSandbox(), tmax, workdir=paths["workdir"])
    )
...
def test_grade_tmax_zeroes_unwritable_reward_path(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    from torchtitan.experiments.rl.examples.tmax import grading
    ...
    paths = _grading_paths(monkeypatch, tmp_path)
    paths["verifier_dir"].mkdir(parents=True)
    paths["reward"].write_text("1")
    # Simulate chattr +i on the directory: the purge cannot delete the
    # pre-written reward and the sentinel cannot land.
    paths["verifier_dir"].chmod(0o555)
    try:
        tmax = {"test_sh": "true", "reward_path": str(paths["reward"])}
        reward = asyncio.run(
            grading.grade_tmax(_GradingSandbox(), tmax, workdir=paths["workdir"])
        ...
        paths["verifier_dir"].chmod(0o755)
